...
经典半导体领域把界面缺陷按其频域响应分为两类（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 16–17，PDF p. 30）：
...
这就是经典的"电容–电压（C-V）法"提取 $D_\text{it}$ 的基本关系（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 17–19）。当缺陷在不同能级上的密度不同时，C-V 测量还可以进一步给出 $D_\text{it}$ 随表面势的依赖关系。
...
其中 $p_i(x)=\dfrac{m^{*}}{\pi\hbar^{2}}\!\displaystyle\int_{-\infty}^{+\infty}\!\dfrac{\mathrm{d}E}{1+\exp[(E-E_F-E_i)/(k_B T)]}$ 是第 $i$ 个子带占据。S-P 自洽过程通过 (4.2)$\to$(4.3) 与 (4.3)$\to$(4.2) 两个方向的迭代来实现；当每轮势能变化小于设定判据时（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 39，使用 $\text{conv}<10\,\mu\text{eV}$），即认为收敛。
...
以此为判据把 $p_\text{it}$ 求出来（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 43–45）。

> 与传统 C-V 法相比，逆向 S-P 路线的优势在于能从一次充电过程中提取"中间态"的界面电荷分布，而不仅限于整个充放电循环的积分响应。

## 隧穿机制：界面态如何被填充

界面缺陷原本位于半导体表面之外的氧化层中，载流子要到达那里必须穿越 SiGe 势垒层。在低温（4.5 K 量级）下，热发射的能量 $k_B T\approx 3.88\times 10^{-4}\,\text{eV}$ 远小于 Ge/SiGe 价带偏移 $\sim 114\,\text{meV}$，因此可以完全忽略热发射的贡献，仅需考虑三种量子隧穿机制（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 49–52）：
...
其中 $N_T$ 是陷阱分布、$\tau$ 是俘获/发射时间常数、$T(x)$ 是每步隧穿系数。在应变锗中位错密度约为 $10^{6}$–$10^{8}\,\text{cm}^{-2}$（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 50），足以提供 TAT 所需的中间态。在低温下，TAT 只需考虑弹性过程，$J_\text{TAT}$ 量级保持在 $10^{-4}$–$10^{-1}\,\text{A/m}^{2}$ 区间，与实验观测的电流衰减一致。
...
分析表明存在一个转变点 $V_g\approx -3.5\,\text{V}$：在 $|V_g|$ 较小时以 TAT 为主导、$|V_g|$ 较大时 F-N 占据主要权重（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 56–57）。

## 对量子点工作点的影响：电荷俘获与漂移

界面态对[[fundamentals/semiconductor-quantum-dot|半导体量子点]]的影响可以归结为三条：

1. **阈值电压漂移与滞回**：被填充的界面态作为固定电荷改变沟道电场，导致阈值电压整体平移。[[materials-devices/strained-germanium|应变锗]] Hall-bar FET 中，未经臭氧处理时阈值电压可漂移至 $\sim -4.84\,\text{V}$，经过钝化后漂移显著减弱（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 33）。
2. **长时间电流衰减**：保持 $V_g$ 不变时源漏电流仍会在 $10^{4}\,\text{s}$ 量级时间窗内指数式衰减至稳态，本质是界面缺陷在此期间被逐步填充——这正是循环扫描阈值电压方法提取 $D_\text{it}$ 的物理基础（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 37）。
3. **低频电荷噪声**：在 GaAs/AlGaAs 体系中，界面缺陷与传统掺杂层的电势涨落叠加产生 $\sim 1/f$ 噪声谱，影响电荷比特与自旋比特的退相干时间。浅刻蚀工艺移除大部分肖特基电极下的二维电子气后，电荷噪声明显抑制（[文献 4]([[sources/you-jie-2016]]) PDF pp. 53–55）。

> 量子点工作点的稳定通常要求"扫描路径不进入强填充区"。一旦 $|V_g|$ 越过历史最大值，新的界面缺陷就会被填上，阈值电压随之整体漂移——这就是为何自动调参算法需要谨慎地设计初始化序列。

## 对谐振腔的影响：损耗角正切与 TLF 耦合

当[[circuit-qed/microwave-resonator|微波谐振腔]]的导体被直接沉积在半导体衬底上时，氧化层中的悬挂键会形成亚稳态二能级涨落子（TLF）。在统计意义上，它们对谐振腔的影响用一个损耗角正切参数 $\tan\delta_\text{TLS}$ 来描述，并直接降低内部品质因子 $Q_i$（[文献 17]([[sources/lin-ting-2022]]) PDF p. 16；[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 65）：
...
由于 $E_\text{rms}(\boldsymbol{r})$ 随 TLF 距腔的距离增大而衰减，距腔约 $100\,\text{nm}$ 以内的 TLF 对 $Q_i$ 才有显著贡献（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 65）。换句话说：
...
| 实验温度 | $\sim 4.5\,\text{K}$，$k_BT\approx 3.88\times 10^{-4}\,\text{eV}$ | 稀释制冷机 | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 50 |
| 直接隧穿电流密度 $J_\text{direct}$ | $\sim 10^{-30}\,\text{A/m}^{2}$ | 应变锗 | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 52 |
| F-N 转变栅压 | $V_g\approx -3.5\,\text{V}$ | 应变锗 Hall-bar | [文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 52, 56 |
| TAT 电流密度范围 | $10^{-4}$–$10^{-1}\,\text{A/m}^{2}$ | 应变锗 | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 55 |
| 异质结中位错密度 | $10^{6}$–$10^{8}\,\text{cm}^{-2}$ | 应变锗 | [文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 49, 55 |
| 陷阱能级（高斯分布均值） | $E_v+140\,\text{meV}$ | 应变锗 SiGe | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 57 |
| TLF 有效耦合距离阈值 | $\sim 100\,\text{nm}$ | 谐振腔–界面 | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 65 |
| 阈值电压漂移幅度 | 漂移至 $\sim -4.84\,\text{V}$（未钝化） | 应变锗 Hall-bar | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 33 |
| 累积 $p_\text{it}$ 与 $V_g$ 线性 Pearson $R$ | $-0.9998$ | 应变锗逆向 S-P | [文献 23]([[sources/ma-jingrui-2026]]) PDF p. 45 |

## 实验特征与测量方法

### 循环扫描阈值电压法（应变锗 / GaAs / Si-MOS）

在 Hall-bar FET 上，先将 $V_g$ 设回零排空界面电荷，再施加从 $0$ 到 $V_\text{min}$ 的电压并保持约 $120\,\text{s}$，让界面态被填充；之后把 $V_g$ 扫回 $0$ 并记录 $I_\text{ds}$–$V_g$ 曲线，提取阈值电压 $V_\text{th}$。随着 $V_\text{min}$ 的绝对值逐步增大，$V_\text{th}$ 整体负向漂移，并最终进入饱和——这表明"大栅压下界面态逐渐被完全填满"。把 $V_\text{min}$ 的扫描范围扩展到 $-9.0\,\text{V}$ 量级，并以 $0.2\,\text{V}$ 为步进，可以获得完整的 $V_\text{th}$–$V_\text{min}$ 曲线（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 32–33）。
...
提取二维空穴气密度 $p_\text{2DHG}$（[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 36–37）。这一方法是逆向 S-P 提取 $p_\text{it}$ 的输入数据源。

### 库仑阻塞区的 $1/f$ 噪声测量

在[[fundamentals/coulomb-blockade|库仑阻塞]]区电子数严格为整数，电荷传感器（如[[readout-measurement/qpc-charge-sensor|QPC]]）的电流随时间涨落直接反映界面态俘获/释放事件。在 GaAs/AlGaAs 体系中，将这一测量与传统掺杂器件对比可以分离"掺杂层势涨落"与"氧化层界面态"两类来源（[文献 4]([[sources/you-jie-2016]]) PDF pp. 64–65）。
...
在应变锗/SiGe 体系上，[文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 62–66 提出了镂空谐振腔设计：通过湿法刻蚀移除超导薄膜下方的部分衬底，并保留四周的支撑柱与一层承托层，使超导薄膜与氧化层界面在垂直方向上拉开微米量级的距离。这样 TLF 的有效耦合按 $Q\sim r$ 显著恢复（[文献 23]([[sources/ma-jingrui-2026]]) PDF p. 65），同时还因为空气进入有效介质降低了等效介电常数，从而提高谐振腔的特征阻抗 $Z_r=\sqrt{L/C}$ 并增强比特–光子耦合 $g\propto(L/C)^{1/4}$。
...
- [[qubit-control/photon-assisted-tunneling|光子辅助隧穿]]：界面态的存在使能级寿命延长，PAT 实验提取的电荷弛豫与退相干时间反映了界面态–载流子相互作用的强度（[文献 4]([[sources/you-jie-2016]]) PDF p. 86）。
...
1. **钝化**：在 Ge 表面预先形成稳定氧化层或沉积钝化膜。例如 [文献 23]([[sources/ma-jingrui-2026]]) PDF pp. 25–28 提出的"臭氧表面钝化"：在异质结表面暴露于臭氧以形成约 1.5 nm 的 SiO<sub>x</sub>，再以 ALD 沉积 30 nm AlO<sub>x</sub> 作为栅氧。钝化后阈值电压漂移显著减弱，$D_\text{it}$ 整体降低。
...
- [[sources/ref-23|文献 23]]，PDF p. 7：摘要级概括——钝化方案、低温界面态建模、含界面态的自洽薛定谔–泊松方程与隧穿机制、镂空腔设计。
- [[sources/ref-23|文献 23]]，PDF pp. 16–17：界面态缺陷模型的微观分类与对器件的三类负面影响（阈值电压漂移、频域失真、复合中心）。
- [[sources/ref-23|文献 23]]，PDF pp. 17–19：电容–电压法提取界面态密度的公式 $C_\text{lf}$、$C_\text{hf}$、$C_\text{it}$ 与 $D_\text{it}\approx C_\text{it}/e^{2}$。
- [[sources/ref-23|文献 23]]，PDF pp. 25–28：臭氧钝化与 AlO<sub>x</sub> 栅氧的制备流程，100 ℃ 干法生长的薄膜厚度与击穿电压统计。
- [[sources/ref-23|文献 23]]，PDF pp. 32–33：循环扫描阈值电压实验流程与 $V_\text{th}$–$V_\text{min}$ 曲线，阈值电压饱和现象。
- [[sources/ref-23|文献 23]]，PDF pp. 36–37：准静态霍尔载流子密度测量，$p_\text{2DHG}$ 与 $24\,000\,\text{s}$ 时间窗内的电流衰减。
- [[sources/ref-23|文献 23]]，PDF pp. 38–40：自洽薛定谔–泊松方程组与离散化后的矩阵形式 $A\boldsymbol{\psi}=\lambda\boldsymbol{\psi}$，牛顿法与 Anderson 方法对比。
- [[sources/ref-23|文献 23]]，PDF pp. 43–45：逆向薛定谔–泊松方法将 $(p_\text{2DHG,th}, E_v(x))=F(V_g, p_\text{it})$ 作为映射，用 Brent 方法搜索 $p_\text{it}$，Pearson $R=-0.9998$ 表明累积 $p_\text{it}$ 与 $V_g$ 高度线性。
- [[sources/ref-23|文献 23]]，PDF pp. 49–52：应变锗异质结中三种隧穿机制的定义，直接隧穿、F-N 隧穿、TAT 的 WKB/Fowler–Nordheim 公式与数值结果。
- [[sources/ref-23|文献 23]]，PDF pp. 55–57：陷阱辅助隧穿的参数扫描与 $J_\text{TAT}$ 估计范围；$V_g\approx -3.5\,\text{V}$ 处 TAT 与 F-N 主导地位的转变。
- [[sources/ref-23|文献 23]]，PDF pp. 62–66：镂空谐振腔设计原理（湿法刻蚀 + 承托层 + 支撑柱），$Q\sim r$ 与 $g\propto(L/C)^{1/4}$ 的两条提升路径。
- [[sources/ref-04|文献 4]]，PDF pp. 53–55：掺杂砷化镓门控量子点低频噪声的来源分析（肖特基电极漏电流）与浅刻蚀量子点工艺。
- [[sources/ref-04|文献 4]]，PDF pp. 64–65：库仑阻塞区低频 $1/f$ 噪声的实验测量与浅刻蚀工艺对噪声的抑制效果。
- [[sources/ref-17|文献 17]]，PDF pp. 15–17：共面波导谐振腔的等效电路、品质因子分解 $1/Q=1/Q_i+1/Q_e$ 与内部损耗的三个来源（准粒子、TLS、寄生电容）。
- [[sources/ref-17|文献 17]]，PDF p. 97：缩短耦合电极在二维电子气平台上的长度可减小由异质结上的二能级系统产生的微波泄露，验证 $Q\sim r$ 的设计原则。
