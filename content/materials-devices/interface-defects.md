...
source_updated: 2026-09-09T03:15:02Z
---
...
3. **低频电荷噪声**：在 GaAs/AlGaAs 体系中，界面缺陷与传统掺杂层的电势涨落叠加产生 $\sim 1/f$ 噪声谱，影响电荷比特与自旋比特的退相干时间。浅刻蚀工艺移除大部分肖特基电极下的二维电子气后，电荷噪声明显抑制。Si/SiGe 中这些 TLF 已可逐个表征（Allan 方差 + 隐马尔可夫建模）并经反馈钉扎——见[[materials-devices/charge-noise|电荷噪声]]词条的单体 TLF 表征与反馈稳定两节。

> 量子点工作点的稳定通常要求"扫描路径不进入强填充区"。一旦 $|V_g|$ 越过历史最大值，新的界面缺陷就会被填上，阈值电压随之整体漂移——这就是为何自动调参算法需要谨慎地设计初始化序列。

### 光照阈值调定与复位（Wolfe 2024）

冷却到低温后界面陷阱电荷被"冻结"，标称相同的器件阈值电压可相差约 **700 mV**——这是阵列器件工作点不可预测的主要来源之一。实验上常用带间光照（光子能量大于带隙）产生电子-空穴对、允许被冻结的电荷重排来"复位"器件，但机制与限度长期缺乏系统研究。Wolfe 等人用 780 nm 近红外激光二极管对无掺杂 Si/SiGe FET 做系统测量，发现一条可设计的规律：**在施加栅偏压 $V_B$ 的同时光照，可把阈值电压设定为一个稳定、可复现的值——宽偏压范围内 $V_T = V_B$**（范围之外仍可调定，只是 $V_T$ 不再等于光照偏压）。一维经典静电模型给出机制：光照产生的载流子让界面陷阱在偏置电场下重新平衡，熄灭后电荷构型被冻结、把阈值"钉扎"在光照偏压上——这也解释了为何光照能复位遭受非期望充电事件的量子点器件：复位不是清除陷阱，而是把陷阱电荷重排到一个由操作者指定的构型。
