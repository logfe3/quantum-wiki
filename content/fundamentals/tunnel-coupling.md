...
把点间隧穿耦合打开之前，[[fundamentals/double-quantum-dot|双量子点]]是两只完全独立的"人造原子"——左右两点的电荷占据数各自服从单点规则，[[fundamentals/charge-stability-diagram|电荷稳定图]]由横竖两组平行直线构成。打开隧穿耦合 $t_c$ 之后，左右两点局域态通过矩阵元 $H_{LR}=\langle L|H|R\rangle=t_c$ 发生杂化，左右两点的波函数在势垒下方渗透一段距离并相干叠加成键态（低能）与反键态（高能），能级差为 $2t_c$——这一点势垒就像把两只原子焊到一起的化学键，于是双点从两个原子变成一个人造分子。这正是文献 20 在其第 2.2.3 节给出的图像："弱隧穿耦合下电子波函数几乎不重叠，系统表现为两个独立的量子点；而在强隧穿耦合下电子波函数显著重叠，形成分子状的结合态和反结合态，导致能级劈裂。"
...
这与[[qubit-control/charge-qubit|电荷量子比特]]所用的哈密顿量一致（见文献 20 式 2.31，文献 21 式 1.7）。
...
其中 $t_{ij}$ 正是量子点 $i$ 与 $j$ 之间的隧穿耦合（跃迁矩阵元），$U_i$ 是点内充电能，$U_{ij}$ 是点间库仑相互作用。CI 模型把 $H$ 中的最后一项（隧穿项）丢掉，于是得到纯经典的蜂巢直线图；保留 $t_{ij}$ 后，三相点附近的电荷转变线就会发生弯曲——直线在反交叉处被"推开"出 $2t_c$ 的能隙，弯曲程度直接标定隧穿耦合的大小。这一点正是文献 21 明确指出的："常相互作用模型无法解释反交叉的弯曲现象，这是因为它只考虑了电容耦合作用"。
...
**(1) 点间电荷转移线拟合。** 在反交叉附近沿失谐方向做精细电荷传感或输运测量，电子占据左点的概率可由 DiCarlo 等给出的热展宽公式描述（文献 21 式 1.8、文献 20 式 5.5）：
...
**(2) 反交叉弯曲的几何读出。** 当三相点附近电荷转变线由直变弯时，其弯曲程度由隧穿耦合 $2t_c$ 与点间库仑相互作用 $V_{12}$ 共同决定。文献 21 给出的双曲线拟合函数为
...
**(3) 微波光谱。** 在电荷量子比特中，$\varepsilon=0$ 处的比特频率 $\omega_q=2t_c$（文献 20 式 4.4 与式 5.7），用两路微波（探测腔 + 驱动）做双色调制谱即可从比特响应峰直接读出 $2t_c/(2\pi)$。当 $2t_c<\omega_q$ 时比特频率与腔频率可分别移动到同一频段，可观察避免交叉与真空 Rabi 劈裂；当 $2t_c>\omega_q$ 时比特与腔的耦合区连续穿越扫频范围，呈现与"$2t_c$ 较小"时显著不同的曲线。
...
1. **势垒栅压呈近指数依赖**：$\ln t_c$ 与势垒电压 $V_B$ 的斜率约为 $\beta\sim 10^{-2}\ \mathrm{mV}^{-1}$（文献 21 给出 $\beta_{12}=6.85\times10^{-2}\ \mathrm{mV}^{-1}$、$\beta_{41}=1.42\times10^{-2}\ \mathrm{mV}^{-1}$，相邻势垒间相差近 5 倍）。
...
3. **次近邻 $t'$ 通常被抑制到接近零**：在 Si/SiGe 二维阵列中，即使最近邻耦合加大到 $t_\text{ave}\sim 280\ \mu\mathrm{eV}$，次近邻 $t_{13}$、$t_{24}$ 仍保持接近零的剩余值（文献 21）；这正是表面码纠错方案所要求的"最近邻连接"模式。

| 量 | 典型值 | 体系 / 条件 | 来源 |
| --- | --- | --- | --- |
| 点间隧穿耦合 $2t_c$ | $25\ \mu\mathrm{eV}$–$120\ \mu\mathrm{eV}$，指数可调 | 电子型非掺杂 GaAs DQD，250 mK | 文献 4 |
| 点间隧穿耦合 $2t_c$ | $10\ \mu\mathrm{eV}$–$100\ \mu\mathrm{eV}$，指数可调 | 电子型非掺杂 GaAs DQD，10 mK | 文献 4 |
| 点间隧穿耦合 $2t_c$ | $\sim 25\ \mu\mathrm{eV}$–$>200\ \mu\mathrm{eV}$，可调至接近零 | Si/SiGe 2×2 阵列的四个最近邻 | 文献 21 |
| 点间隧穿耦合 $2t_c$ | $2.7$、$6.8$、$11\ \mathrm{GHz}$（对应 $11.2$、$28.2$、$45.6\ \mu\mathrm{eV}$） | Si/SiGe DQD，分别 $V_M=0.42$、$0.444$、$0.452\ \mathrm{V}$ | 文献 4 |
| 点间隧穿耦合 $2t_c$ | $2t_c/(2\pi)=9.4\ \mathrm{GHz}$（$\sim 39\ \mu\mathrm{eV}$） | Si/SiGe TQD 中的 RDQD 翻转模式比特 | 文献 20 |
| 点间隧穿耦合 $2t_c$ | $2t_c/(2\pi)=8.0\ \mathrm{GHz}$（$\sim 33\ \mu\mathrm{eV}$） | Si/SiGe TQD 中的 LDQD 翻转模式比特 | 文献 20 |
| 共振交换比特隧穿耦合 | $t_l=12.15\ \mathrm{GHz}$、$t_r=12.35\ \mathrm{GHz}$ | Si/SiGe TQD，RX 比特工作位 | 文献 20 |
| 势垒调控指数 $\beta$ | $1.4\times10^{-2}$–$6.9\times10^{-2}\ \mathrm{mV}^{-1}$ | Si/SiGe 2×2 阵列，势垒 B12/B23/B34/B41 | 文献 21 |
| 充电能 $E_C$ | $2.83$、$5.02$、$3.05$、$4.63\ \mathrm{meV}$ | Si/SiGe 2×2 阵列，四个量子点（虚拟栅极提取） | 文献 21 |
| 杠杆臂 $\alpha$ | $\sim 0.12\ \mathrm{eV/V}$ | Si/SiGe 2×2 阵列，平均值 | 文献 21 |
| 电子温度 $T_e$ | $\sim 80\ \mathrm{mK}$（无微波）；$\sim 280\ \mathrm{mK}$（微波加热） | 无液氦稀释制冷机，非掺杂 GaAs | 文献 4 |
| 电子温度 $T_e$ | $\sim 150\ \mathrm{mK}$（拟合下界） | Si/SiGe 2×2 阵列，20 mK | 文献 21 |
| 拟合最低可分辨 $t_c$ | $\sim 30\ \mu\mathrm{eV}$ | SET 偏置 0.2 mV、激励 200 µV | 文献 21 |
...
**(2) 单态–三重态与交换比特。** 在[[qubit-control/singlet-triplet-qubit|单态–三重态量子比特]]中，交换相互作用由虚跃迁产生，文献 21 引用的标准结果为
...
**(3) 共振交换量子比特。** 在三量子点中编码的[[qubit-control/resonant-exchange-qubit|共振交换（RX）量子比特]]中，三个电子分布在三个点之间，比特频率主要由隧穿耦合决定。文献 20 在 $t_l=t_r=t$ 且 $\varepsilon=\Delta=0$ 时给出 $\omega_\text{RX}\approx 0.52\,t$；实验拟合得到 $t_l/(2\pi)=12.15\ \mathrm{GHz}$、$t_r/(2\pi)=12.35\ \mathrm{GHz}$，从而 $\omega_\text{RX}/(2\pi)=6.22\ \mathrm{GHz}$。

**(4) 与腔的耦合强度。** 隧穿耦合通过混合角调制有效自旋–光子耦合强度（文献 20 式 2.56）：

$$
g_\text{eff}=g_0\sin\theta=g_0\cdot\frac{2t_c}{\hbar\omega_q},
$$

其中全局耦合 $g_0=\tfrac{1}{2}\beta\omega_r\sqrt{Z_r/\pi\hbar}$，正比于谐振腔特征阻抗 $Z_r$ 的平方根。因此，提升隧穿耦合是把电偶极矩"传递"给自旋比特的关键途径——这是文献 20 用 Si/SiGe TQD 配合高阻抗腔（$Z_r\approx 3.5\ \mathrm{k\Omega}$）实现自旋–光子强耦合（$2g_s/2\pi=43.5\ \mathrm{MHz}$）的微观机理。
...
**(1) 通过势垒栅指数调节。** 势垒栅每改变数十毫伏，$t_c$ 即可在 $\mu\mathrm{eV}$ 到 $\mathrm{meV}$ 量级跨越近 4 个数量级；但同一势垒栅往往也对相邻点的电化学势产生串扰，需要[[scaling-automation/virtual-gates|虚拟电极]]对势垒方向与柱塞方向做正交化。文献 4 在非掺杂 GaAs 双量子点上以中间栅极 $V_T$ 或 $V_M$ 实现了 $2t_c$ 的指数型连续调节，文献 21 在 Si/SiGe 2×2 阵列上对四个最近邻势垒分别给出了 $\beta_{ij}$ 的数值。

**(2) 关断次近邻、保留最近邻。** 表面码纠错要求次近邻隧穿耦合被完全抑制。在文献 21 的器件中，中心势垒 CB 电压为零时，$t_{13}$、$t_{24}$ 的拟合值接近零；通过将 CB 电压加到 250 mV 可以单独打开 $t_{24}$、关闭 $t_{13}$，从而把阵列从"方形"切换为"菱形"耦合结构，为模拟阻挫磁性与几何阻挫提供平台。

**(3) 微波辅助隧穿谱。** 把微波驱动加到势垒电极或柱塞电极上时，隧穿耦合可由[[qubit-control/photon-assisted-tunneling|光子辅助隧穿]]（PAT）共振线之间的距离提取。文献 4 用 PAT 共振条件 $\alpha\varepsilon=\sqrt{(hf)^2-(2t_c)^2}$ 反演得到 $2t_c$，与电荷转移线拟合的结果互相校验；文献 13、文献 17 的 JC 模型章节中也给出了等价的微波频谱推导。
...
- [[fundamentals/charge-stability-diagram|电荷稳定图]]上的反交叉弯曲直接由隧穿耦合决定：文献 21 明确指出"CI 模型无法解释反交叉的弯曲现象"。弯曲程度是 $t_c$ 的最常用几何读数，也是[[scaling-automation/charge-state-recognition|自动调点]]中判断"欠耦合 / 适中 / 过耦合"的特征量。
...
- [[sources/ref-04|文献 4]]，PDF p. 4：非掺杂 GaAs 双量子点中通过下层栅压调节实现 $2t_c$ 的指数变化，把双点从两个分立量子点连续转变为一个大量子点。
- [[sources/ref-04|文献 4]]，PDF p. 79：5.3 节介绍电子型非掺杂砷化镓双量子点的基本结构与电极布局（T-L、T-R 为势垒、T-M 控制耦合、LP/RP 为柱塞）。
- [[sources/ref-04|文献 4]]，PDF pp. 81–82：电子温度标定与 DiCarlo 模型拟合（P(1,0) 公式 5-1 在 PDF p. 82），$T_e\approx 300\ \mathrm{mK}$、$\alpha_L=0.047\ \mathrm{meV/mV}$。
- [[sources/ref-04|文献 4]]，PDF p. 82：$2t_c$ 随门电极电压 $V_T$ 指数增加，由 $25\ \mu\mathrm{eV}$ 调到 $120\ \mu\mathrm{eV}$。
- [[sources/ref-04|文献 4]]，PDF pp. 87–88：6.2 节双量子点耦合强度 $2t_c$ 随中间电极 $V_M$ 的变化曲线（PDF p. 88 图 6.2）及指数拟合（$10\ \mu\mathrm{eV}$–$100\ \mu\mathrm{eV}$）。
- [[sources/ref-04|文献 4]]，PDF pp. 89–90：6.3 节光子辅助隧穿中共振峰拟合公式 $\alpha\varepsilon=\sqrt{(hf)^2-(2t_c)^2}$ 与 $2t_c=2.7$、$6.8$、$11\ \mathrm{GHz}$ 的提取。
- [[sources/ref-21|文献 21]]，PDF p. 8：摘要说明 2×2 阵列同时具有最近邻与次近邻隧穿耦合，且均可独立调节。
- [[sources/ref-21|文献 21]]，PDF p. 30：Hubbard 模型哈密顿量（式 1.10）与"点间隧穿耦合与充电能分别对应动能项与库仑相互作用项"的明确表述，1.2.4 节系统建立隧穿耦合与交换项的对应关系。
- [[sources/ref-21|文献 21]]，PDF p. 27：电荷二能级系统的哈密顿量 $H=(\varepsilon/2)\sigma_z+t\sigma_x$、本征能量与 DiCarlo 拟合公式 1.8，以及反交叉弯曲与 $2t_c$ 的双曲线关系式 1.9；明确指出 CI 模型无法解释反交叉弯曲。
- [[sources/ref-21|文献 21]]，PDF pp. 81–82：双量子点 QD1-QD2 的反交叉点（红色圈）随势垒栅 $V_{B12}$ 增加而增大；势垒栅同时增强隧穿耦合与点间库仑相互作用。
- [[sources/ref-21|文献 21]]，PDF pp. 87–89：4.4.2 节基于同步扫描同时观察 QD1-QD4、QD2-QD3、QD1-QD3、QD2-QD4 四个双量子点的反交叉；4.4.3 节最近邻 $t_{ij}$ 从约 $25\ \mu\mathrm{eV}$ 到超过 $200\ \mu\mathrm{eV}$ 可调，势垒调控系数 $\beta_{ij}$ 范围 $1.4\times10^{-2}$–$6.9\times10^{-2}\ \mathrm{mV}^{-1}$。
- [[sources/ref-21|文献 21]]，PDF pp. 90–92：4.4.4 节次近邻隧穿耦合的调控方法——PDF p. 91 双曲线拟合式 4.3 与"vCB=250 mV 时打开 $t_{24}$、保持 $t_{13}$ 关闭"的非对称选择性打开。
- [[sources/ref-21|文献 21]]，PDF p. 95：4.B 节 DiCarlo 拟合的准确性讨论（式 4.4–4.5）——SET 偏置 0.2 mV、激励 200 µV 时测量下限约 $30\ \mu\mathrm{eV}$。
- [[sources/ref-20|文献 20]]，PDF p. 8：摘要说明三量子点与谐振腔耦合器件同时具备电荷和自旋自由度，并通过谐振腔探测到四相点、量子元胞自动机以及电荷比特、翻转模式比特、RX 比特与腔的强耦合。
- [[sources/ref-20|文献 20]]，PDF p. 35：2.2.4 节给出电荷量子比特哈密顿量 $\hat H=(\varepsilon/2)\hat\sigma_z+t_c\hat\sigma_x$（式 2.31），隧穿耦合速率 $2t_c$ 与混合角 $\theta=\arctan(2t_c/\varepsilon)$ 的对应关系。
- [[sources/ref-20|文献 20]]，PDF p. 88：6.2 节双量子点隧穿耦合 $2t_c$ 随中间门电极电压 $V_M$ 的指数拟合曲线，可由 $10\ \mu\mathrm{eV}$ 调至 $100\ \mu\mathrm{eV}$。
- [[sources/ref-20|文献 20]]，PDF pp. 39–40：耦合强度计算 2.3.2 节给出 $g_\text{eff}=g_0\sin\theta=g_0\cdot 2t_c/(\hbar\omega_q)$（式 2.56），混合角 $\theta=\arctan(2t_c/\varepsilon)$，以及全局耦合 $g_0=\tfrac{1}{2}\beta\omega_r\sqrt{Z_r/\pi\hbar}$ 的推导。
- [[sources/ref-20|文献 20]]，PDF pp. 60–61：电荷量子比特隧穿线附近谐振腔相位信号——$V_{B3}=90\ \mathrm{mV}$ 时 $2t_c<\omega_q$（两次跳跃），$V_{B3}=230\ \mathrm{mV}$ 时 $2t_c>\omega_q$（无跳跃）。
- [[sources/ref-20|文献 20]]，PDF pp. 74–77：双色调制谱测量电荷量子比特隧穿耦合——PDF p. 74 给出 $2t_c/(2\pi)=9.0\ \mathrm{GHz}$（$\gamma_c/(2\pi)=47.2\ \mathrm{MHz}$），PDF p. 77 给出 RDQD 自旋比特工作位的 $2t_c/(2\pi)=9.4\ \mathrm{GHz}$，PDF p. 86 总结 LDQD/RDQD 两组参数；隧穿耦合与 $V_{B3}$ 近似成线性关系。
- [[sources/ref-20|文献 20]]，PDF pp. 98–99：6.3.2 节共振交换量子比特能谱——RX 比特频率 $\omega_\text{RX}\approx 0.52\,t$（$t_l=t_r=t$、$\varepsilon=\Delta=0$ 时），实验拟合 $t_l=12.15\ \mathrm{GHz}$、$t_r=12.35\ \mathrm{GHz}$，$\omega_\text{RX}/(2\pi)=6.22\ \mathrm{GHz}$。