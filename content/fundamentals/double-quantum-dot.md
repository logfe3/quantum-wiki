...
*双量子点器件结构与电子传输示意图（[文献 19]([[sources/yang-jiecheng-2023]])，图 1.3）*

<!-- 原始占位：串联双量子点示意图：左右两点经互电容 $C_m$、互电阻 $R_m$ 耦合，各自经柱塞栅 $V_{g1}$、$V_{g2}$ 调能级；SEM 实物图标注源、漏、柱塞与势垒栅 -->

## 两种几何：串联与并联

按两点的空间排列与电子输运路径，双量子点分两类（文献 1论文 1.4.3 节）：
...
串联双量子点的经典图像是一个**电容–电阻网络**（文献 1论文式 1.5；文献 6论文式 1.17–1.21）。左点 1 与右点 2 分别通过电容 $C_{g1}$、$C_{g2}$ 与柱塞栅 $V_{g1}$、$V_{g2}$ 耦合，通过电容 $C_L$、$C_R$ 与源、漏电子库耦合；两点间经**互电容** $C_m$ 与**隧穿电阻** $R_m$ 相互作用耦合。每个点的总电容为
...
![[assets/figures/fundamentals/zhou-cheng-2013-fig1-5-dqd-schematic-and-sem.jpg]]

*串联/并联双量子点的电路示意与扫描电镜照片（[文献 1]([[sources/zhou-cheng-2013]])，图 1.5）*

<!-- 原始占位：双量子点能级示意图：横轴失谐 ε，透热基 |L⟩、|R⟩（虚线交叉）与绝热基 |±⟩（实线反交叉）；反交叉能隙 2tc、混合角 θ 与本征能量 E± -->

### 静电能与电化学势

完整描述需要回到常相互作用框架。设左、右点电子占据数分别为 $(N_1,N_2)$，静电能为（文献 6论文式 1.17）
...
两点各自的[[fundamentals/electrochemical-potential|电化学势]]定义为向该点增加一个电子所需的最小能量（文献 6论文式 1.22–1.23）：
...
零偏压下，电子依次通过双点的输运条件 $\mu_S=\mu_1=\mu_2=\mu_D$ 只在蜂窝图三条边的交汇处满足，称为**三相点**（triple point）。三相点分两类（文献 6论文图 1.10(d)）：
...
文献 12论文在 Ge 纳米线双量子点中（$V_{SD}=2\ \mathrm{mV}$）直接量出偏压三角形边长，给出 $\alpha_L=0.14$、$\alpha_R=0.13$；进一步提取 $C_{G2}=6.7\ \mathrm{aF}$、$C_{G4}=6.2\ \mathrm{aF}$，$C_L\approx C_R\approx 47\ \mathrm{aF}$，对应充电能 $E_{CL}\approx 4.3\ \mathrm{meV}$、$E_{CR}\approx 4.6\ \mathrm{meV}$。

偏压进一步增大时激发态进入窗口，三角形内部出现与边平行的激发态电导线，可用于读出[[qubit-control/singlet-triplet-qubit|单态–三重态量子比特]]等体系的能级间距。文献 12论文中即在偏压三角形底部观测到 $T(2,0)$–$S(2,0)$ 的能级差 $\Delta_{ST}\approx 1.1\ \mathrm{meV}$。
...
| 量 | 典型值 / 范围 | 来源 |
| --- | --- | --- |
| 反交叉能隙 $2t_c$（电荷比特） | $20.7\ \mu\mathrm{eV}\approx 5\ \mathrm{GHz}$（GaAs，门控定义） | 文献 1（拉比频率标定） |
| 充电能 $E_{C1},E_{C2}$ | $1.5\ \mathrm{meV}$（浅刻蚀 GaAs 单点）；$4$–$6\ \mathrm{meV}$（锗硅纳米线空穴点） | 文献 4、文献 10 |
| 充电能 $E_{C1},E_{C2}$（双量子点） | $4.3\ \mathrm{meV}$、$4.6\ \mathrm{meV}$（Ge 纳米线双点） | 文献 12 |
| 点间隧穿 $t_c$（自旋阻塞提取） | $45\pm 5\ \mu\mathrm{eV}$（Ge 纳米线双点，自旋轨道 $t_{SO}=27\pm 3\ \mu\mathrm{eV}$） | 文献 12 |
| 杠杆臂 $\alpha_i=C_{gi}/C_i$ | $0.14$、$0.13$（Ge 纳米线双点） | 文献 12 |
| 栅电容 $C_{gi}$ | $6.7\ \mathrm{aF}$（左）、$6.2\ \mathrm{aF}$（右）（Ge 纳米线双点） | 文献 12 |
| 耦合电容比 $C_m/C_{1(2)}$ | $\sim 0$（弱）$/0.5$（人造分子区）$/1$（强） | 文献 1、文献 6 |
| $(0,2)$ 单态–三重态能级差 $E_{ST}$ | 数百 $\mu$eV（GaAs 约 $400\ \mu\mathrm{eV}$）；Ge 双点约 $1.1\ \mathrm{meV}$（多空穴） | 文献 6、文献 12 |
| Ge 纳米线双点朗德 $g$ 因子 | $3.4\pm 0.2$ | 文献 12 |
| 工作温度 | $30\ \mathrm{mK}$（稀释制冷机） | 文献 1（GaAs） |

## 实验特征与测量

### 电荷稳定图

双量子点最常用的"地图"是[[fundamentals/charge-stability-diagram|电荷稳定图]]：扫描两个柱塞栅压 $V_{g1},V_{g2}$，记录使系统能量最低的电荷组态 $(N_1,N_2)$。它分弱、中、强三种形态（文献 1论文图 1.7）：
...
文献 12论文在 Ge 纳米线双量子点中演示了"高度可调"的隧穿耦合：在固定 $V_{SD}=0.5\ \mathrm{mV}$ 时，把中间栅 $V_{G3}$ 从 $0\ \mathrm{V}$ 调到 $-0.04\ \mathrm{V}$，可以直接把蜂窝图从清晰锐利的分立三相点形态过渡到模糊蜂窝、再到典型蜂窝——整个形态演化即 $t_c$ 标定的图像化。

<!-- FIGURE: 棚顶型 Ge 纳米线双量子点的可调耦合演化：$V_{G3}=0,-0.02,-0.04\ \mathrm{V}$ 三幅相图对比 -->

### 自旋阻塞与自旋–电荷转换

把自旋自由度纳入图像后，双量子点出现一种标志性的实验现象——**泡利自旋阻塞**（Pauli spin blockade，PSB）。它由两个事实共同导致（文献 6论文 2.2.2 节）：
...
- **脉冲栅法**：不依赖电流，先在 $(0,1)$ 区初始化、再到加载点无自旋选择地装入第二个电子（$S$ 与三个 $T$ 近似等概率），最后脉冲到 $(0,2)$ 区内的测量点停留大部分周期；当测量点落在由三条电荷跃迁延长线围成的"阻塞三角形"内时，QPC 信号介于 $(1,1)$ 与 $(0,2)$ 之间——证明部分时间系统被阻塞在 $(1,1)$。文献 6论文样品上由信号衰减拟合给出三重态弛豫时间约 $70\ \mu\mathrm{s}$；
- **偏压三角形法**：文献 12论文在 Ge 纳米线双量子点中比较正反偏压下的三角形：正向偏压三角形底部基态线上电流被抑制，发生 PSB；反向偏压三角形畅通无阻。从阻塞三角形宽度读出 $\Delta_{ST}\approx 1.1\ \mathrm{meV}$；在不同磁场下重复此测量，得到 $\Delta_{ST}(B)=\Delta_{ST}(0)-g\mu_B B$，线性拟合给出 $g=3.4\pm 0.2$。

非零磁场下三重态 $T_\pm$ 进一步发生塞曼劈裂，漏电流随磁场呈"谷形"特征。Ge 纳米线双量子点的强自旋轨道耦合使 $S$、$T$ 在外磁场下混合解除阻塞，产生漏电流的双峰结构。文献 12论文用
...
- **光子辅助隧穿**与**LZSM 干涉**：在双量子点失谐上叠加单色或脉冲驱动，重复穿越反交叉得到相干干涉条纹——文献 2论文在 GaAs 双量子点上观察到 14 阶光子过程，并证明其本质是 LZS 干涉；
...
- [[sources/ref-01|文献 1]]，PDF pp. 26–28：1.4.3 节给出双量子点定义（人造分子）与串联、并联两类几何（式 1.5、$C_{1(2)}$），以及 $C_m/C_{1(2)}\sim 0,\,0.5,\,1$ 三种耦合强度下的蜂窝图演化（图 1.5–1.7）。
- [[sources/ref-01|文献 1]]，PDF pp. 28–31：1.5 节把电子占据双量子点不同位置视作二能级系统，给出最小哈密顿量与布洛赫球编码，能级反交叉 $E_\mathrm{gap}=2|\Delta|$。
- [[sources/ref-01|文献 1]]，PDF pp. 32–34：1.6.2 节把双量子点列为自旋比特、电荷比特与 singlet–triplet 比特的共同物理载体（图 1.11）。
- [[sources/ref-01|文献 1]]，PDF pp. 107–112：双量子点中单电子电荷量子比特的最小哈密顿量 $H=(\varepsilon/2)\sigma_z+\Delta\sigma_x$、$|L\rangle,|R\rangle$ 基矢、反交叉能隙 $2\Delta=20.7\ \mu\mathrm{eV}$ 由拉比振荡标定，以及高斯脉冲驱动的 LZS 干涉与布洛赫球任意角度旋转。
- [[sources/ref-01|文献 1]]，PDF pp. 123–124：GaAs/AlGaAs 双量子点电荷比特样品参数——二维电子气形成于表面下 $95\ \mathrm{nm}$、电子密度 $3.2\times 10^{11}\ \mathrm{cm^{-2}}$、电子迁移率 $1.5\times 10^5\ \mathrm{cm^2/Vs}$、量子点几何尺寸约 $250\ \mathrm{nm}$。
- [[sources/ref-06|文献 6]]，PDF pp. 20–21：1.2.4 节给出双量子点静电能 $U(N_1,N_2)$ 与充电能 $E_{C1},E_{C2},E_{Cm}$ 公式（式 1.17–1.21）以及电化学势 $\mu_1,\mu_2$（式 1.22–1.23）。
- [[sources/ref-06|文献 6]]，PDF pp. 21–22：弱、中、强三种耦合下的蜂窝图演化，三相点的电子型与空穴型两种类型，偏压三角形与杠杆臂 $\alpha_1,\alpha_2$（式 1.24–1.25）。
- [[sources/ref-06|文献 6]]，PDF pp. 33–36：自旋阻塞的两种观测（输运法与脉冲栅法）；正反偏压不对称即 PSB 指纹；脉冲栅法测得三重态弛豫时间约 $70\ \mu\mathrm{s}$。
- [[sources/ref-06|文献 6]]，PDF pp. 29–30：2.2.1 节 $(1,1)$–$(0,2)$ 反交叉的两能级哈密顿量、失谐 $\varepsilon$、隧穿耦合 $\Delta$ 与单态–三重态量子比特工作哈密顿量（式 2.9）。
- [[sources/ref-12|文献 12]]，PDF pp. 13–14：双量子点电路模型、电化学势 $\mu_L,\mu_R$ 公式（式 1.3–1.4）、零偏压下 $C_m=0$ 时的方格相图与 $C_m>0$ 时的蜂窝图演化（图 1.2(c)(d)）。
- [[sources/ref-12|文献 12]]，PDF p. 14：偏压三角形的形成与内部激发态输运线（图 1.3）。
- [[sources/ref-12|文献 12]]，PDF pp. 53–54：Ge 纳米线双量子点样品结构（五根 Ti/Pd 顶层栅极，$V_{G3}$ 调节隧穿耦合）；不同 $V_{G3}$ 下相图从分立三相点到典型蜂窝的过渡（图 3.1–3.2）。
- [[sources/ref-12|文献 12]]，PDF pp. 54–55：$V_{SD}=2\ \mathrm{mV}$ 偏压三角形给出 $C_{G2}=6.7\ \mathrm{aF}$、$C_{G4}=6.2\ \mathrm{aF}$、$\alpha_L=0.14$、$\alpha_R=0.13$、$E_{CL}\approx 4.3\ \mathrm{meV}$、$E_{CR}\approx 4.6\ \mathrm{meV}$（图 3.2(d)）。
- [[sources/ref-12|文献 12]]，PDF pp. 55–56：Ge 双量子点中泡利自旋阻塞——正反偏压三角形底部基态电流被抑制，从阻塞宽度提取 $\Delta_{ST}\approx 1.1\ \mathrm{meV}$（图 3.3）。
- [[sources/ref-12|文献 12]]，PDF pp. 56–57：磁场下 $\Delta_{ST}(B)=\Delta_{ST}(0)-g\mu_B B$ 线性拟合给出 $g=3.4\pm 0.2$（图 3.4）；漏电流随磁场的双峰谷形结构由强自旋轨道耦合驱动，拟合得到 $t\approx 45\pm 5\ \mu\mathrm{eV}$、$t_{SO}\approx 27\pm 3\ \mu\mathrm{eV}$（图 3.5，式 3.1）。