...
| 量 | 典型量级 | 说明 |
| --- | --- | --- |
| 总电容 $C_\Sigma$ | $\sim 10$–$100\ \mathrm{aF}$ | 百纳米门控量子点；浅刻蚀 GaAs 单点实测 $113\ \mathrm{aF}$（文献 4） |
...
- **隧穿耦合导致的反交叉**：在三相点附近，点间[[fundamentals/tunnel-coupling|隧穿耦合]] $t_C$ 使 $(N_1,N_2)$ 与 $(N_1-1,N_2+1)$ 态杂化，电荷转变线发生弯曲、张开 $2t_C$ 的能隙。CI 模型不含任何隧穿矩阵元，无法解释这种弯曲——蜂窝图中直线的弯曲程度恰恰是提取 $t_C$ 的手段（文献 21 明确指出这一局限）；
...
- [[sources/ref-05|文献 5]]，PDF pp. 22–23：单量子点 CI 模型的两条假设、$U(N)$ 与 $\mu(N)$ 公式（式 1-1、1-2）、充电能与加电子能定义。
- [[sources/ref-05|文献 5]]，PDF pp. 25–28：双量子点静电能 $U(N_1,N_2)$、$E_{C1}/E_{C2}/E_{Cm}$ 公式、$C_m\to0$ 与强耦合两个极限、蜂窝图与三相点、边距公式 $\Delta V_g=e/C_g$ 与 $\Delta V_g^m$。
- [[sources/ref-05|文献 5]]，PDF p. 29：常相互作用模型为经典模型，量子态修正使峰周期含 $1+\Delta E/E_C$ 因子。
- [[sources/ref-06|文献 6]]，PDF pp. 14–17：CI 模型假设、单点等效电路，以及电容矩阵推导（式 1.1–1.13），证明电源做功恰好抵消相互作用项、点内项即量子点自由能。
- [[sources/ref-06|文献 6]]，PDF pp. 20–22：串联双点等效电路与静电能、蜂窝图构建与三相点四重简并条件。
- [[sources/ref-07|文献 7]]，PDF pp. 18–20：CI 模型能量表达式、充电能为化学势与静电势之和、多电子区充电能近似相等故库仑峰等间距。
- [[sources/ref-16|文献 16]]，PDF pp. 35–36：CI 模型 $U(N)=Q^2/2C+\sum E_n$ 与加电子能 $E_\mathrm{add}=e^2/C+\Delta E$。
- [[sources/ref-16|文献 16]]，PDF p. 39：Hubbard 模型作为 CI 模型的扩展，CI 结果只含势能项与库仑排斥项、不含自旋与隧穿项。
- [[sources/ref-21|文献 21]]，PDF p. 27：常相互作用模型无法解释三相点附近反交叉弯曲，需量子力学模型提取隧穿耦合。
