...
描述半导体量子点最常用的两个互补模型是[[fundamentals/constant-interaction-model|常相互作用模型]]（CI 模型）与扩展 Fermi-Hubbard 模型。前者是经典电路视角，把多体相互作用压缩为总电容；后者是量子力学视角，把动能的隧穿项也纳入。前者描述阻塞与稳定图，后者描述隧穿线弯曲与阵列多体态。相互作用谱系的第三站是密度/相互作用比再增大后的[[fundamentals/wigner-molecule|Wigner 分子]]——电子不再弥散而是自组织成空间分立的"晶体"构型；多电子点的中间地带则可由含谷自由度的 Hartree-Fock 建模衔接（见 CI 词条"超越 CI"一节）。
