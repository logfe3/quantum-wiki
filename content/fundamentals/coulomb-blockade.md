...
多电子区 $\Delta E\ll E_C$ 时峰严格等周期。这解释了为什么栅压扫描中阻塞谷与导通峰交替等距出现，也说明峰周期本身就是栅电容的直接度量。峰间距的**统计涨落**则超出[[fundamentals/constant-interaction-model|常相互作用模型]]+随机矩阵理论的预言——硅 MOS 量子点上实测涨落为 Wigner 猜想的 15 倍以上、随 $E_C$ 而非 $\Delta E$ 标度，见该词条"经典统计检验"一节。同理，$\mu(N)$ 对 $V_{SD}$ 的斜率为 $-|e|C_S/C_\Sigma$，源、漏、栅三个方向斜率的差异正是库仑菱形倾斜边界的来源。
