...
其中 $U(N)$ 是含 $N$ 个电子时系统的总能量。$\mu(N)$ 同时包含了两个不可分割的部分——经典充电能 $E_C=e^2/C_\Sigma$（点电容储能）和单粒子能级 $E_N$（束缚势决定的分立能），所以它既是"化学势"，也是"静电势"；韩天一论文中明言"$\mu_N$ 即化学势和静电势之和"，陈宝宝、余国栋、尤杰等人也从不同起点给出完全等价的定义。
...
其中 $\Delta E=E_{N+1}-E_N$ 是单粒子能级间隔。余国栋论文中强调，$E_\mathrm{add}$ 是"每向量子点中增加一个电子所需要增加的能量"，可理解为填充一个电子的综合代价。对百纳米量级量子点，$C_\Sigma$ 很小，$E_C\gg\Delta E$，电化学势近似等间距排列，库仑峰近似等周期出现；少电子区（最后几个电子）则 $\Delta E$ 不可忽略，加电子能随 $N$ 起伏，直接泄露单粒子能谱。
...
电子既不能从源极跳入也不能从漏极跳出，顺序隧穿被禁止，进入[[fundamentals/coulomb-blockade|库仑阻塞]]区。韩天一论文 1.5 节明确指出，"阻塞区电子数目保持不变"，这一性质是实验上逐个数出量子点中电子数、把量子点用作"人造原子"的基础。
...
CI 模型是纯经典理论。余国栋、刘汉伟论文明确指出，CI 模型无法解释三相点附近电荷转移线的弯曲。引入第 $n$ 个单粒子能级修正后，向点 $i$ 中能级 $n$ 填充一个电子所需电化学势可以写成经典电化学势与单粒子能级之和 $\mu_{i,n}=\mu_i^{c}+E_n$；这一修正把加电子能改写为
...
其中 $U_i=e^2(C_i+\sum_j C_{ij})/C_\Sigma^2$，$U_{ij}=e^2 C_{ij}/C_\Sigma^2$，与 CI 模型在去掉隧穿项时完全对应；刘汉伟论文 1.36–1.38 式明确给出该映射，并指出"考虑 $H_t$ 时不同的 $t_{ij}$ 取值会导致反交叉点出现轻微的偏移"——这是自动调点算法判定点间耦合强弱的物理依据。
...
| 杠杆臂 $\alpha$（GaAs） | plunger gate $0.05$，barrier gate $0.1$（无量纲） | 尚汝南 2014 |
| 杠杆臂 $\alpha$（GaAs） | $0.027$–$0.06\ \mathrm{meV/mV}$（不同电极、不同器件） | 尤杰 2016 |
| 充电能 $E_C$（Ge/Si 纳米线空穴点） | $4$–$6\ \mathrm{meV}$ | 李炎 2018 |
| 杠杆臂 $\alpha$（Ge/Si 纳米线空穴点） | $0.20$–$0.25\ \mathrm{eV/V}$ | 李炎 2018 |
| 栅–点电容 $C_G$ | $C_{RP}=3.1\ \mathrm{aF}$，$\alpha_{RP}=0.027\ \mathrm{meV/mV}$ | 尤杰 2016 |
| 阻塞前提 | $E_C\gg k_\mathrm{B}T$ 且 $G\ll 2e^2/h$ | 尚汝南 2014 |
...
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 7：研究背景与基本概念综述，电化学势、充电能、库仑阻塞、库仑菱形等核心概念的引出。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 22：CI 模型两条假设、单点总能量 $U(N)$（式 1-1）与电化学势 $\mu(N)$（式 1-2）的推导，$E_C=e^2/C$ 定义。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 23：加电子能 $E_\mathrm{add}=\mu(N+1)-\mu(N)=E_C+\Delta E$（式 1-3），库仑振荡与阻塞图像（图 1.5）。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 25：双量子点串联结构、CI 模型推广与等效电路（图 1.8）。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF pp. 26–28：双点静电能 $U(N_1,N_2)$（式 1-5）与三个特征能量 $E_{C1},E_{C2},E_{Cm}$（式 1-7~1-9），$C_m\to 0$ 与 $C_m/C_{1(2)}\to 1$ 两个极限，蜂窝图与三相点的构建。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 27：双点电化学势 $\mu_1(N_1,N_2)$、$\mu_2(N_1,N_2)$（式 1-12、1-13）及加电子能的物理含义。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF pp. 28–29：蜂窝图原胞结构（图 1.10）、边距公式 $\Delta V_g=|e|/C_g$（式 1-18、1-19）与耦合导致的 $\Delta V_g^m$（式 1-22、1-23）；并指出 CI 模型为经典模型，需要量子修正。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 29：含单粒子能级的量子修正加电子能（式 1-24）及修正后的栅压周期（式 1-25、1-26）。
- [[sources/yu-guodong-2016|余国栋 2016]]，PDF p. 30：隧穿耦合 $t_C$ 使三相点附近转移线弯曲（图 1.11），以及源漏偏压下三相点展成偏压三角形（图 1.12）。
- [[sources/you-jie-2016|尤杰 2016]]，PDF p. 4：博士论文核心研究目标，包括电化学势、库仑阻塞与库仑菱形等基础概念的引出。
- [[sources/you-jie-2016|尤杰 2016]]，PDF pp. 16–17：单量子点 CI 模型（式 1-1、1-2），电化学势定义（式 1-3），加电子能 $E_\mathrm{add}=E_C+\Delta E$（式 1-4），库仑阻塞与库仑振荡的关系。
- [[sources/you-jie-2016|尤杰 2016]]，PDF p. 22：电化学势、充电能、库仑阻塞、库仑菱形四个基本参数与现象的总结。
- [[sources/you-jie-2016|尤杰 2016]]，PDF pp. 56–58：串联双量子点静电能、$E_{C1}/E_{C2}/E_{Cm}$ 公式（式 3-9~3-11），双点电化学势 $\mu_1,\mu_2$（式 3-14、3-15），耦合能 $E_{Cm}$ 的定义（式 3-16）。
- [[sources/you-jie-2016|尤杰 2016]]，PDF p. 57：不同耦合强度下电流蜂窝图的演化（图 3.2）：弱耦合方格、中间耦合蜂窝、强耦合单点极限。
- [[sources/you-jie-2016|尤杰 2016]]，PDF p. 61：三相点附近沿失谐方向扫描构建二能级体系，构成电荷量子比特工作点的实验演示（图 3.6）。
- [[sources/you-jie-2016|尤杰 2016]]，PDF p. 80：浅刻蚀 GaAs 量子点实测 $C_\Sigma=113\ \mathrm{aF}$、$E_C=1.5\ \mathrm{meV}$、$C_{RP}=3.1\ \mathrm{aF}$、$\alpha_{RP}=0.027\ \mathrm{meV/mV}$。
- [[sources/shang-runan-2014|尚汝南 2014]]，PDF p. 18：单量子点中电子数与电化学势的关系，库仑阻塞与化学势相对位置的物理图像（图 1.3）。
- [[sources/shang-runan-2014|尚汝南 2014]]，PDF p. 54：库仑菱形测量杠杆臂的方法，GaAs 量子点 plunger gate $\sim0.05$、barrier gate $\sim0.1$ 的参考值。
- [[sources/zhou-cheng-2013|周诚 2013]]，PDF pp. 25–26：CI 模型的两条假设、$U(N)$ 与 $\mu(N)$ 公式、库仑阻塞条件 $\mu(N-1)<\mu_D<\mu_S<\mu(N)$，以及 $G\ll 2e^2/h$ 的阻塞前提。
- [[sources/zhou-cheng-2013|周诚 2013]]，PDF p. 28：不同耦合强度 $C_m/C_{1(2)}\sim 0,\ 0.5,\ 1$ 下双量子点电荷稳定图的形态演化。
- [[sources/chen-baobao-2017|陈宝宝 2017]]，PDF p. 17：用电容矩阵推导单点电化学势（式 1.15），证明电源做功恰好抵消相互作用项，点内项即量子点自由能。
- [[sources/chen-baobao-2017|陈宝宝 2017]]，PDF pp. 20–22：串联双点等效电路、双点电化学势 $\mu_1/\mu_2$（式 1.22、1.23）、蜂窝图的构建、电子型与空穴型三相点。
- [[sources/han-tianyi-2017|韩天一 2017]]，PDF pp. 19–20：电化学势定义（式 1.2）、化学势与静电势之和的物理含义、零偏压库仑振荡与偏压窗口图像，以及 $E_C=e|V_{SD}|$ 提取和 $\alpha=|V_{SD}|/\Delta V_g$ 标定。
- [[sources/liu-hanwei-2022|刘汉伟 2022]]，PDF p. 34：CI 模型下单点电化学势（式 1.22），库仑阻塞态与电流的对应关系。
- [[sources/liu-hanwei-2022|刘汉伟 2022]]，PDF pp. 38–39：Hubbard 模型哈密顿量（式 1.36–1.38）作为 CI 模型的扩展，电化学势项 $H_\mu$、跃迁项 $H_t$、库仑排斥项 $H_U$、交换作用项 $H_J$ 的分解；CI 模型只含势能项与库仑排斥项，不含自旋与隧穿项。
- [[sources/li-yan-2018|李炎 2018]]，PDF pp. 57–58：锗硅纳米线空穴量子点中空穴隧穿过程的电化学势解释，激发态落入偏压窗口使电流增加。
- [[sources/li-yan-2018|李炎 2018]]，PDF pp. 58–60：空穴量子点库仑菱形测量流程、$E_C=4$–$6\ \mathrm{meV}$、$\alpha=0.20$–$0.25\ \mathrm{eV/V}$，磁场下基态与激发态的塞曼劈裂区分奇偶空穴数。
