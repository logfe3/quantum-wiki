...
- **工作点优化**：调整外磁场方向、栅压偏置使比特对电荷噪声的灵敏度最小化，本身即可延长 $T_2^*$ 与 $T_2^\mathrm{Hahn}$； 在锗空穴体系中找到一个最优磁场方向，使 $T_2^\mathrm{Hahn}$ 提升约 $5$ 倍、量子门集层析（GST）报告的单比特门错误率下降 50%。DD 与工作点优化并非互斥：前者对"剩余低频噪声"起作用，后者把"低频噪声到比特的耦合通道"先压缩。
