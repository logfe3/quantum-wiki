...
即"自旋轨道长度"。$l_\mathrm{SO}$ 越短，单位电场位移产生的有效自旋转动越大，EDSR 越快。论文中徐刚 2020 报告锗棚顶纳米线双量子点 $t_\mathrm{SO}\sim 38\pm 4\ \mu\mathrm{eV}$、$l_\mathrm{SO}\sim 40{-}100\ \mathrm{nm}$，与 GaAs 电子体系 $l_\mathrm{SO}\gtrsim 1\ \mu\mathrm{m}$ 形成鲜明对比。
...
锗空穴体系 $f_\mathrm{Rabi}$ 可达数百 MHz 到数 GHz 量级，远高于电子自旋比特常报告的 1–30 MHz。论文中报告的最高值：徐刚 2020 棚顶纳米线 $f_\mathrm{R}\sim 698\pm 2\ \mathrm{MHz}$；周雨晨 2026 平面应变锗几何门区 $f_\mathrm{Rabi}=8{-}19\ \mathrm{MHz}$ 用于容错优化的"低速率高保真"区。
...
论文中王柯 2020 在锗棚顶纳米线单空穴比特上用 Hahn echo 把退相干从 $T_{2}^{*}\sim 65\ \mathrm{ns}$ 提升到 $T_{2}^\mathrm{Hahn}\sim 523\ \mathrm{ns}$，并实现 $\pi$ 脉冲单比特翻转保真度超过 99%；周雨晨 2026 报告在最优工作点下 $T_2^\mathrm{Rabi}\sim 1.77\ \mu\mathrm{s}$。
...
- **腔色散读出**：把比特耦合到超导微波腔，用比特–腔色散位移读取电荷或自旋态；徐刚 2020 报告锗棚顶纳米线空穴双量子点–腔耦合强度 $g_C/2\pi\sim 15\ \mathrm{MHz}$。
...
| 量 | 典型值 | 来源 |
| --- | --- | --- |
| 自旋轨道耦合强度 $t_\mathrm{SO}$ | $\sim 38\pm 4\ \mu\mathrm{eV}$ | 徐刚 2020（锗棚顶纳米线双量子点） |
| 自旋轨道长度 $l_\mathrm{SO}$ | $40{-}100$ nm | 徐刚 2020；GaAs 电子 $l_\mathrm{SO}\gtrsim 1\ \mu\mathrm{m}$ |
| 单比特 Rabi 频率（高速） | $f_\mathrm{R}\sim 698\pm 2\ \mathrm{MHz}$ | 徐刚 2020 |
| 高速区 $T_{2}^{\mathrm{Rabi}}$ | $\sim 1.77\ \mu\mathrm{s}$（@ $f_\mathrm{Rabi}\sim 19\ \mathrm{MHz}$） | 周雨晨 2026 |
| 优化工作点 $T_{2}^{*}$ 提升 | 约 5 倍（相对常规工作点） | 周雨晨 2026 |
| 几何门单比特保真度 | $>99.9\%$（$f_\mathrm{Rabi}=8{-}19\ \mathrm{MHz}$ 区间均 $>99\%$） | 周雨晨 2026 |
| 动力学门优化后保真度 | 最高 $99.82\%$，超过容错阈值 | 周雨晨 2026 |
| Hahn echo 提升 | $T_{2}^{*}\sim 65\ \mathrm{ns}\to T_{2}^\mathrm{Hahn}\sim 523\ \mathrm{ns}$ | 王柯 2020 |
| 空穴–腔耦合强度 | $g_C/2\pi\sim 15\ \mathrm{MHz}$ | 徐刚 2020 |
| 重空穴有效质量 | 比电子小（具体值与应变/限域相关，论文强调"对量子点尺寸要求相对宽松"） | 周雨晨 2026 |
| 单比特门操作时间（参考） | 20 ns 单比特、75 ns 两比特（锗异质结） | 马璟睿 2026 转引文献 [36] |
| 单/两比特保真度（参考） | 99.97% 单比特、99.3% 两比特（空穴跳跃方法） | 马璟睿 2026 转引文献 [41] |
| 阵列规模（参考） | $2\times 2$、$2\times 4$、$4\times 4$、10、18 空穴比特 | 马璟睿 2026 转引文献 [43–47] |
| 长程穿梭距离 | 自旋态在 9 μm（可优化至 49 μm）穿梭后保持相干 | 马璟睿 2026 转引文献 [48] |

> [!note] 上表第三栏"参考"标记的数据来自马璟睿 2026（PDF pp. 18–19）转引的国际同行工作，不是本站 26 篇学位论文的原始内容，使用时请核对原文。
...
- [[sources/xu-gang-2020|徐刚 2020]]，PDF pp. 4–5：锗棚顶纳米线空穴 EDSR、腔耦合与定位纳米线探索。
- [[sources/wang-ke|王柯 2020]]，PDF pp. 9–10：锗棚顶纳米线单空穴自旋比特 Rabi、Larmor 与 Hahn echo。
- [[sources/zhou-yuchen-2026|周雨晨 2026]]，PDF pp. 7–9：平面锗几何门、SOC 矢量场与最优工作点。