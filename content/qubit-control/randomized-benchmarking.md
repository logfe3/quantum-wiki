...
<div class="entry-lead">量子过程层析能重构一个门的完整矩阵，但资源随比特数指数增长，且无法把态制备与测量（SPAM）误差和门误差分开。随机化基准测试（randomized benchmarking，RB）换了一条路：对被测比特施加一串随机 Clifford 门再加一个把态转回初态的恢复门，随机化把噪声对称化成去极化信道，回退概率便随门数做干净的指数衰减——衰减率只反映门本身的平均误差。这个"搅匀再读衰减率"的思想衍生出一整个方法家族，是自旋比特、超导比特门保真度的标准量尺。</div>
