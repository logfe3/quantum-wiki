...
- **远程网络与认证**：transmon 也是分布式架构的基本节点——相距 30 m 的两台制冷机之间经微波量子链路分发纠缠并完成设备无关自检验，见[[superconducting-qubits/remote-qubit-self-testing|远程超导比特自检验]]。
- **多层构型变体**：平面多层真空隙 cQED 中的"孔径 transmon"——比特作为谐振器导电边界的开孔、以离面电场跨层耦合，见[[scaling-automation/mmiqc-3d-integration|多层微波集成量子电路]]的平面多层路线一节。
...
2. **介质与界面损耗**：电场落在无定形介质、表面氧化层和界面缺陷上会耦合两能级缺陷。大电容只在电场主要储存在低损耗区域时才真正有利。界面处理的工艺杠杆（HF→NH4F 湿法刻蚀对 T1 与 TLS 密度的大样本统计改善）见[[superconducting-qubits/surface-tls-loss|表面 TLS 损耗与湿法刻蚀]]词条。
...
- [[superconducting-qubits/fluxonium-qubit|Fluxonium 量子比特]]是与之互补的路线：同样依赖大 $E_J/E_C$ 压低电荷色散，但用大电感重塑势阱，在小电容下保留数百 MHz 的非谐性；两条路线在非谐性、磁通噪声和读出频移上各有取舍；把 transmon 直流接到高阻抗传输线上则进入[[circuit-qed/superstrong-coupling|超强耦合]]区——其四次非谐被稀释到 Γρ 个线性模上。
...
- transmon 结势垒内还有一族[[superconducting-qubits/junction-defect-modes|约瑟夫森结强耦合缺陷模]]：结面积线性缩放的共振 TLS 数目直接进入结设计规则（小结面积 + 高临界电流密度），而结区清洗工艺的选择主导品质因子——两者共同决定 $T_1$ 的材料预算。transmon 的三能级结构（qutrit）还能反过来利用：驱动-耗散下把 {|e⟩,|f⟩} 子空间做成非厄米比特、以 |g⟩ 为环境，n 个比特在 Ω=γ/4 处出现 2ⁿ 阶奇异点并加速多体纠缠生成，见[[readout-measurement/pt-symmetry-parametric-amplifier|参量放大器中的 PT 对称性破缺]]词条"高阶推广"一节。
