...
进而求出裸速率 $\Gamma^\mathrm{out},\Gamma^\mathrm{in}$；假设它们与温度无关，即可外推任意 $T_e$ 下的读出表现。结论是：阈值无关方法的 $A_\mathrm{eff}$ 随温度升高而缩小，在 $B=1.5\ \mathrm{T}$ 下约 $0.7\ \mathrm{K}$ 处与传统方法持平——传统阈值法因 $E_z/T_e>13$ 的要求在 $100\ \mathrm{mK}$ 以上就难以工作，而阈值无关方法把可用的温度上限提高了近一个量级。作为参照，Si-MOS 实验中 $150\ \mathrm{mK}$ 的电子温度已把自旋单发读出可见度压到约 0.7，成为自旋–电荷转换保真度的主要瓶颈。
