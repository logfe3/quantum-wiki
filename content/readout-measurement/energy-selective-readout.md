...
随后源漏以 $|\!\downarrow\rangle$ 速率回填一个低能电子，把量子点重新占据为基态。这一"激发态先出、基态后入"的单次事件，在传感器上产生一个特征的高电平脉冲（blip）；如果电子本来就处于 $|\!\downarrow\rangle$，信号一直保持低电平。两种轨迹一一对应，电子自旋被转换为可由阈值判定的电荷跳变（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 34；[文献 22]([[sources/chu-ning-2025]])，PDF pp. 38–39）。

<!-- FIGURE: E–W–R 三段脉冲波形与量子点能级示意图：Empty（费米面高于两能级，量子点排空）→ Wait（费米面低于两能级，随机自旋电子载入）→ Readout（费米面介于两能级之间，↑ 跳出、↓ 留下） -->

读出窗口（readout window）由塞曼能 $E_Z$ 决定。窗口越宽，塞曼能级对热涨落越不敏感，但需要的驱动频率也越高：硅中 $g\approx 2$，$1\ \mathrm{T}$ 磁场对应的拉莫尔频率约 $28\ \mathrm{GHz}$（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 34）。这是[[qubit-control/single-spin-qubit|单自旋比特]]实验中微波驱动与读出窗口之间的"高频代价"。
...
| 读出保真度（SiMOS） | $V^R\approx 85\%$（文献 15） | $>99\%$（[文献 21]([[sources/wang-ning-2025]])，PDF p. 32） |

能量选择性读出的优势是原理最简单、对量子点结构要求最低；代价是依赖源漏费米面，因此读出保真度受电子温度限制。PSB 不依赖源漏，可达更高温度与更高保真度，目前是硅基多比特扩展的标准读出方案（[文献 22]([[sources/chu-ning-2025]])，PDF p. 127；[文献 21]([[sources/wang-ning-2025]])，PDF p. 95）。
...
读出阶段的动力学涉及三个状态——"激发态未被排出" $P_\uparrow$、"基态未被排出" $P_\downarrow$、"量子点已被排出过" $P_0$——状态矢量 $\mathbf{P}=(P_\uparrow,P_\downarrow,P_0)^T$ 满足 $d\mathbf{P}/dt=Q\mathbf{P}$，转移矩阵为（[文献 15]([[sources/hu-ruizi-2022]])，PDF pp. 66–67）：
...
其中 $\Gamma_\downarrow^\mathrm{tot}=\Gamma_\downarrow^\mathrm{out}+\Gamma_\downarrow^\mathrm{in}$。第一项与 $P_\uparrow^I$ 无关，是基态电子来回隧穿构成的"本底鼓包"；第二项与 $P_\uparrow^I$ 成正比，是平均信号上"自旋鼓包"（spin bump）的来源，实验上把这条曲线拟合即可提取三个隧穿速率（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 66；[文献 22]([[sources/chu-ning-2025]])，PDF p. 39）。

### 自旋–电荷转换可见度

若只关心时间窗口 $t_r$ 内电子是否跳出过——即忽略回填通道（$\Gamma_\downarrow^\mathrm{in}=0$），自旋–电荷转换可见度的解析式为（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 66）
...
完整的读取可见度严格满足因子化关系（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 68）
...
| 量 | 典型值 | 来源 |
| --- | --- | --- |
| 外磁场 $B_\mathrm{ext}$ | $1.5\ \mathrm{T}$（SiMOS，单发自旋） | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 电子温度 $T_e$ | $180.5\pm 8.1\ \mathrm{mK}$（稀释制冷机） | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 塞曼–温度比 $E_Z/(k_BT_e)$ | $11.22$（限制 $V^{STC}$） | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 自旋弛豫速率 $1/T_1$ | $112\pm 6\ \mathrm{s^{-1}}$（$T_1\approx 8.9$ ms） | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
...
| SET 电阻（$\uparrow$ / $\downarrow$） | $625\ \mathrm{k\Omega}$ / $500\ \mathrm{k\Omega}$（SiMOS） | [文献 22]([[sources/chu-ning-2025]])，PDF p. 127 |
| 寄生电容 $C_p$ | $0.2\ \mathrm{pF}$（SiMOS 仿真） | [文献 22]([[sources/chu-ning-2025]])，PDF p. 127 |
| 最佳时间窗口 $t_\mathrm{opt}^{STC}$ | $0.84\ \mathrm{ms}$ | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 最佳时间窗口 $t_\mathrm{opt}^R$ | $0.90\ \mathrm{ms}$ | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 最大 $V^{STC}$ | $97.15\%$ | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| 最大 $V^R$ | $85.4\%$ | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 64 |
| 阈值无关 $A_\mathrm{eff}$ 提升 | $\times 60$ | [文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70 |
| Elzerman 读出时间（Si/SiGe） | $\sim 5$ ms | [文献 21]([[sources/wang-ning-2025]])，PDF p. 111 |
| 单比特读出时间（两比特联合） | $28$ ms（$1000$ 次平均） | [文献 22]([[sources/chu-ning-2025]])，PDF p. 87 |

要让 $V^{STC}$ 最大值超过 $99\%$，需要同时满足三个条件（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70）：

$$
E_Z/(k_B T_e) > 13,\qquad T_1\cdot\Gamma_\uparrow^\mathrm{out} > 100,\qquad \Gamma_s/\Gamma_\downarrow^\mathrm{in} > 12.
$$

第一个条件最难满足——它要求电子温度足够低（$T_e< E_Z/13\,k_B$）或外磁场足够大，对应 $1\ \mathrm{T}$ 磁场下驱动场频率约 $28\ \mathrm{GHz}$（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 34）。在 SiMOS 样品中 150 mK 电子温度下自旋单发读出信号的可见度只有 $\sim 0.7$（[文献 22]([[sources/chu-ning-2025]])，PDF p. 143），是限制读出保真度的关键瓶颈。

## 实验特征

### E–W–R 三段脉冲序列

硅量子点单发读出的典型时间序列由三段方波组成（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 34；[文献 22]([[sources/chu-ning-2025]])，PDF p. 74）：
...
并非任意读出位置都能实现自旋选择性读出：电压太高时两自旋态都能跳出，电压太低时两自旋态都不跳出。标定方法（[文献 22]([[sources/chu-ning-2025]])，PDF p. 39）：用 E–W–R 序列固定 Empty/Load 电压而扫描 Readout 高度，再跟两个连续的 Load–Read 作为监视位（无论第一位读出结果如何，后面两位的电子状态必然回到 $|\!\downarrow\rangle$，因此监视位的"高电平比例" $>0.2$ 即说明暗计数过多，需重新标定）。合适的读出位置会出现 50:50 的高低电平分布，平均信号 $x(t)$ 呈"自旋鼓包"（spin bump）形状——这是 Elzerman 读出的指纹特征。

### 单发读出的速率方程拟合流程

完整模拟分三步（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 59）：
...
直流 SET 测量的带宽受直流线缆寄生电容与锁相积分时间限制，仅数十 kHz 量级（[文献 7]([[sources/han-tianyi-2017]])，PDF p. 38），不足以追踪 Elzerman 读出中 $\sim\ \mathrm{ms}$ 量级的隧穿事件细节。把 SET 接入阻抗匹配谐振电路，以[[readout-measurement/rf-reflectometry|射频反射]]方式读出，构成 RF-SET，把带宽推到 MHz–百 MHz 量级，可逐次追踪单次隧穿事件。

硅基 SiMOS 中典型 RF-SET 设计：贴片电感 $L$ 与寄生电容 $C_p$ 组成谐振电路，把 SET 的高电阻（$R_\mathrm{SET}\sim 625\ \mathrm{k\Omega}$）匹配到 $50\ \Omega$ 传输线；通过劈裂栅极（split-gate）方法在累积栅极 AL1 上连接射频端口，把 $C_g$ 从"泄漏通道"转化为"耦合通道"，阻抗匹配的 $C_g$ 范围扩大到 $0.1$–$5\ \mathrm{pF}$（[文献 22]([[sources/chu-ning-2025]])，PDF pp. 127–128）。

### 与泡利自旋阻塞的扩展联合

在 GaAs 双量子点的 (1,3)–(0,4) 区附近，文献 6等人发现一种非阻塞区读出机制：把三重态 $T(1,3)$ 的弛豫路径引导到比单态少一个电子的中间亚稳态 $(0,3)$，从而把单态映射为 $(0,4)$、三重态映射为 $(0,3)$。$(0,3)$–$(0,4)$ 的电荷差通过 QPC 传感后是 $(1,3)$–$(0,4)$ 的 1.6 倍，这种"信号放大"把读出信号增强到自旋阻塞方案的 2 倍左右（[文献 6]([[sources/chen-baobao-2017]])，PDF pp. 66–73）。尽管这条机制仍以电荷占据作为媒介，但其核心思想——通过控制弛豫路径放大自旋–电荷转换效率——是能量选择性读出思想的多电子区推广。

### 多比特联合与读出时间代价

Elzerman 读出与自旋穿梭（spin-shuttle）结合可推广到两比特体系：右比特 $Q_R$ 不直接与源漏电子库耦合，而是先把电子隧穿到左比特 $Q_L$，再在 $Q_L$ 端与源漏费米面交换完成读出（[文献 22]([[sources/chu-ning-2025]])，PDF pp. 86–87）。这种"左点代理"读出牺牲读出时间换取结构兼容性，整体单发读出时间约 28 ms（$1000$ 次平均）。
...
- [[readout-measurement/threshold-independent-readout|阈值无关读出]]以能量选择性读出为载体，把可见度 $V^R$ 的下降显式反推回 $P_\uparrow^I$，把 $|P_\uparrow^E/P_\uparrow^I-1|<1\%$ 的参数空间扩大约 60 倍（[文献 15]([[sources/hu-ruizi-2022]])，PDF p. 70）。
...
- 在多电子编码与二维扩展的趋势下，Elzerman 读出逐渐被 PSB + RF-reflectometry 取代（[文献 22]([[sources/chu-ning-2025]])，PDF p. 143；[文献 21]([[sources/wang-ning-2025]])，PDF p. 111），但在初始化、低温基准测量、远距离自旋搬运等场景仍是首选。
...
- [[sources/ref-15|文献 15]]，PDF p. 34：1.3.2–1.3.3 节系统阐述自旋–电荷转换两条主要路径——能量选择性隧穿（Elzerman）与泡利自旋阻塞（PSB），给出窗口大小、温度适用范围的对比；明确"主误差源是源漏费米面热展宽"。
- [[sources/ref-15|文献 15]]，PDF pp. 59–65：第 3 章研究背景与单发读出信号处理流程（GMM 拟合、速率方程、Monte-Carlo 模拟、贝塞尔滤波），给出最大 $V^R=85.4\%$ 的实验值。
- [[sources/ref-15|文献 15]]，PDF pp. 66–67：3.3.3–3.3.4 节给出读出阶段的速率方程与转移矩阵 $Q$、$P_0(t)$ 解析解、自旋–电荷转换可见度 $V^{STC}$ 的解析式与最佳时间窗口 $t_\mathrm{opt}^{STC}=0.84$ ms。
- [[sources/ref-15|文献 15]]，PDF p. 68：3.3.6 节严格推导 $V^R=V^{STC}\times V^E$ 的因子化关系，并由此得到 $V^E=V^R/V^{STC}$ 的解析反推。
- [[sources/ref-15|文献 15]]，PDF p. 70：参数表给出实测隧穿速率 $\Gamma_\uparrow^\mathrm{out}=6.0\pm 0.1$ kHz、$\Gamma_\downarrow^\mathrm{out}=27\pm 2$ Hz、$\Gamma_\downarrow^\mathrm{in}=1.39\pm 0.04$ kHz、$\Gamma_s=50$ kHz、$T_e=180.5\pm 8.1$ mK、$1/T_1=112\pm 6$ s$^{-1}$、最佳 $t_\mathrm{opt}^{STC}=0.84$ ms、最大 $V^{STC}=97.15\%$；并给出 $V^{STC}>99\%$ 的三个条件 $E_Z/T_e>13$、$T_1\cdot\Gamma_\uparrow^\mathrm{out}>100$、$\Gamma_s/\Gamma_\uparrow^\mathrm{in}>12$。
- [[sources/ref-15|文献 15]]，PDF p. 70：3.4 节阈值无关读出方法——读取可见度与测量概率的线性关联 $P_\uparrow^M=P_\uparrow^I V^R+\alpha$、反推公式 $P_\uparrow^E=(P_\uparrow^M-\alpha)/V^R$、有效参数空间扩大 60 倍的定量结果。
- [[sources/ref-22|文献 22]]，PDF pp. 38–39：2.2.1 节"Elzerman 读取"详细推导硅基体系中基于源漏电子交换的读出方案，给出图 2.1 中 Si-MOS 的电荷稳定图（排空、加载、读出三阶段圆点）与三种情况下的电化学势变化；并指出"后续实验中排空阶段可省去，因为读取后电子已初始化到 $|\!\downarrow\rangle$"。
- [[sources/ref-22|文献 22]]，PDF p. 39：2.2.2 节自旋单发读出位置的标定方法——排空–加载–读取三级脉冲序列加两个连续监视位、扫描 Readout 高度的 spin-bump 特征、"自旋向上和自旋向下比例理论上是 50:50"。
- [[sources/ref-22|文献 22]]，PDF p. 74：图 4.3(c) 测量方案示意图——在读出阶段（10 ms）执行能量选择性读出，$|\!\uparrow\rangle$ 电子隧穿到电子储存器并加载 $|\!\downarrow\rangle$ 电子完成初始化；$|\!\downarrow\rangle$ 电子不发生跃迁。
- [[sources/ref-22|文献 22]]，PDF pp. 86–87：5.2 节把单比特 Elzerman 读取与自旋穿梭技术结合推广到两比特体系——右比特 $Q_R$ 通过左比特 $Q_L$ 与源漏费米面进行电子交换；整体单发读出时间约 28 ms、$1000$ 次平均。
- [[sources/ref-22|文献 22]]，PDF pp. 127–128：6.3.2 节 RF-SET 结构设计与劈裂栅极方法仿真——SET 阻抗在自旋向上与自旋向下时分别为 $625\ \mathrm{k\Omega}$ 与 $500\ \mathrm{k\Omega}$，对应 RTS 高低电平；阻抗匹配 $C_g$ 范围 $0.1$–$5\ \mathrm{pF}$，远宽于欧姆方法的 $C_g=0$ 唯一值。
- [[sources/ref-22|文献 22]]，PDF p. 143：7.2.1 节自旋比特读出的优化讨论——Elzerman 读出速率受 $\Gamma$ 限制、保真度受 150 mK 电子温度影响（可见度 $\sim 0.7$）；PSB 读出可在无电子库情况下进行、点间隧穿耦合 $t_c$ 通常远大于点–库隧穿速率 $\Gamma$、结合射频反射可达更快速度与更高保真度。
- [[sources/ref-21|文献 21]]，PDF p. 32：1.3.2 节把能量选择性读出与 PSB 直接对比——Elzerman 依赖于源漏电子库、保真度容易受电子温度影响、需要线路滤波抑制热涨落；PSB 可在更高温度下工作、读出及初始化保真度可超过 $99\%$、自旋–电荷转换过程快（μs 量级）。
- [[sources/ref-21|文献 21]]，PDF p. 95：6.2.1 节展望读出方法优化——本论文中 Elzerman 读出时间为 5 ms，远大于自旋比特操控时间；未来采用 PSB + 射频反射测量是国际上普遍采用的方案。
- [[sources/ref-06|文献 6]]，PDF pp. 66–73：第 4 章"双量子点中自旋状态的增强读取"提出 (1,3)–(0,4) 区亚稳态中继机制——把三重态 $T(1,3)$ 的弛豫路径引导到 $(0,3)$ 亚稳态，把单态映射为 $(0,4)$、三重态映射为 $(0,3)$，从而把 QPC 互导信号放大 2 倍；并提取 $T_\mathrm{RO}=1.1\ \mathrm{\mu s}$、$T_\mathrm{RI}=10\ \mathrm{\mu s}$、$T_1=89\ \mathrm{\mu s}$ 等特征时间。
- [[sources/ref-07|文献 7]]，PDF pp. 23, 62：参考文献与综述明确给出 Elzerman 等人 2004 年 GaAs 自旋单发读出的原始文献，并阐述 RF-QPC / RF-SET 是从直流 SET 出发提升带宽的工程方案。
