...
*440 单元 SNAIL-TWPA 的增益谱：蓝线为实测、黄线为 HB 仿真（泵浦 8.5 GHz，实验输入泵浦功率 $-97\ \mathrm{dBm}$，仿真信号电流取泵浦的 1/1000）。如 3WM 预期，泵浦位于增益谱高端之外；谱纹波源于端口阻抗失配的多次反射。图源：Shiri et al. (2024), Fig. 14。*增益纹波与环境的耦合并非 TWPA 独有：谐振式 JPA 与环形器失配构成的有效法布里–珀罗腔同样产生纹波并可用输入输出模型定量整形（平顶宽带增益），见[[readout-measurement/parametric-amplifier|参量放大器]]的"环境法布里–珀罗干涉"一节——两类器件的纹波诊断与相位建模共用同一套物理。放大器族的另一分支是把非线性摊到薄膜上的动力学电感器件与单步光刻的 IEJPA，见[[readout-measurement/kinetic-inductance-twpa|动力学电感行波参量放大器]]与[[readout-measurement/parametric-amplifier|参量放大器]]词条。
