...
- **射频反作用**：过高的射频功率会向量子点注入散粒噪声与热激发，破坏被测态，需要折中灵敏度与反作用）。除这类经典功率反作用外，传感耦合本身还有量子反作用——库仑耦合构成 which-path 探测通道、以可定标的速率侵蚀被测系统相干性，其机制与能标见[[readout-measurement/charge-sensor-backaction|电荷传感反作用与 which-path 退相干]]。
