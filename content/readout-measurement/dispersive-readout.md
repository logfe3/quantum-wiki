...
- **不可避免的反作用**：探测光子既提供信息也是扰动来源——测量诱导退相干（measurement-induced dephasing）、AC Stark 频移、以及经腔辐射的Purcell 通道都会与读出速度、信噪比（SNR）和保真度直接挂钩。

```mermaid
flowchart LR
  QD["双量子点状态<br/>|g⟩ 或 |e⟩"] -->|"色散耦合 χ"| R["高阻抗微波谐振腔"]
  R --> G["|g⟩：腔频 ωr-χ"]
  R --> E["|e⟩：腔频 ωr+χ"]
  G --> IQ["反射/透射 I-Q 信号"]
  E --> IQ
...
_量子点状态通过相反方向的腔频平移映射到 I-Q 平面，读出链不需要让比特与腔发生真实激发交换。_
...
是色散频移（dispersive shift）。与光子数 $n=a^\dagger a$ 成正比的 $2\chi n$ 称为交流斯塔克频移，与光子数无关的 $\chi$ 来自真空涨落，称为兰姆频移。
...
在半导体量子点体系中，可用高阻抗谐振腔把多个双量子点接到共享微波模式，并用频率规划或多个读出谐振器实现复用。在线性近似下，相位响应随参与耦合的量子点数增加；当多个跃迁同时接近腔模时，局域破坏性干涉可能压低总响应，因此需要错开工作频率并校准每个量子点的色散贡献。
...
- **Purcell 弛豫**：量子点激发态混入 $(g/\Delta)$ 比例的腔光子成分，该成分以 $\kappa$ 速率漏出腔外，给出 $\gamma^{\mathrm{Purcell}}\simeq\kappa_r g^2/\Delta^2$ 的额外弛豫通道；

因此读出速度、SNR 与反作用必须联合优化。量子点读出链应同时控制腔线宽、探测功率和首级放大器噪声，并在外加磁场下验证增益与动态范围。
...
- 色散频移 $\chi$ 与腔线宽 $\kappa$ 在读出速度和 Purcell 弛豫之间形成基本矛盾——Purcell 滤波器通过引入频率选择性阻抗零点解决这一矛盾。
