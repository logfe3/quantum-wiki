...
由于这套方法把比特态编码到一段独立可观测的相位或频率信号上，它在 2004 年 Wallraff 等人首次演示[[circuit-qed/circuit-quantum-electrodynamics|cQED]] 架构时就被用作主读出手段，并被沿用至半导体量子点–腔杂化体系）。这套布居测量能力反转过来就是[[readout-measurement/qubit-thermometry|超导比特温度计]]：热平衡下三能级布居比直接编码 60–200 mK 的环境温度。它也能当机器学习的前端：把数据编码为腔位移幅度、读 Fock 态占据概率作非线性特征，同一套色散硬件即构成[[circuit-qed/quantum-reservoir-computing|量子储备池计算]]的实验载体。读出端的环境工程还有干涉式方案：[[circuit-qed/giant-atom|人工巨原子]]量子滤波器在保持大 $\kappa_r$ 快读出的同时把比特寿命延长十倍。
