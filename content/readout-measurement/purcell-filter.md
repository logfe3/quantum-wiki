...
原则上，减小腔耗散率 $\kappa_r$ 或增大比特–腔失谐 $\Delta$ 都能压低 Purcell 速率，但这与快速高保真度读出的要求直接冲突：快读要求腔线宽大（光子进出腔快）、色散频移 $\chi$ 大（信噪比随 $\chi$、$\kappa$ 增长），而这恰恰使 Purcell 弛豫变强。Purcell 滤波器（Purcell filter）正是为打破这一矛盾而引入的附加滤波电路：它让读出谐振频率附近的微波顺利通过到达放大链，同时在比特频率附近制造阻带或阻抗零点，降低比特经读出端口的自发辐射。它不是简单地把整条读出线"关小"——否则读出同样会变慢——而是利用比特与腔之间天然的频率间隔（通常数 GHz）做频率选择性隔离。滤波不必是固定频域的：[[circuit-qed/giant-atom|人工巨原子]]方案用两点干涉做频率**且**功率选择的"量子滤波器"——对比特衰变单光子反射、对强读出信号透明，在 $\kappa_r/2\pi$ 很大时仍把 $T_1$ 延长十倍量级。
...
**可调谐宽带滤波**把"收窄抗噪"与"宽带快读"合二为一：滤波器内嵌 SQUID，磁通调谐其频率与带宽——**空闲态**把滤波器调离谐振带并收窄带宽（$\kappa_r\ll\chi$），光子噪声诱导退相干被抑制 **7 倍**；**测量态**把滤波器调到与谐振带共振（$\kappa_r\approx2\chi$），快速读出。实测：100 ns 脉冲单发保真度 **99.6%**（受读出期间弛豫限制），多能级读出协议（同时分辨 $|0\rangle/|1\rangle/|2\rangle$）把弛豫极限也绕开——**50 ns 达 99.9%**；三比特同时 100 ns 读出平均保真度 **99.5%**、串扰低；QND 性能 99.4%（重复测量）、泄漏率 <0.1%。同一论文还给出基于可调滤波的表面码 QEC 可扩展读出架构。

![[assets/figures/purcell-filter/3ce6b82f254e6f6ee85d7b50bc62e076176132c8cf1600dad72725bd33be5816.jpg]]

*可调谐宽带 Purcell 滤波器的读出保真度表征：单发读出的积分直方图——空闲/测量双态切换的滤波器配合多能级协议，50 ns 脉冲达 99.9% 保真度。图源：Xiong et al. (2025)，Fig. 3(a)。*
...
**干涉式抑制**是滤波路线族谱的最后一块：读出谐振器在**两个空间分离的点**耦合到馈线——比特模式经两路传到馈线时**相消干涉**，不需要任何专用滤波元件或阻抗失配。实测抑制超过 **2 个量级**（400 MHz 带宽、腔线宽 13.8 MHz），凹陷频率处 Purcell 寿命 **>16 ms**（抑制因子 >2000）。全通（all-pass）读出架构因此可行——信号双向通过而比特仍受保护。

![[assets/figures/purcell-filter/5d1082ff2d27335d87a52c3f88b9b6c1864ddda0f82bafe25c014bde63196c39.jpg]]

*干涉式 Purcell 滤波器原理：谐振器两点耦合馈线——电容与电感耦合的电压贡献在馈线两端相消干涉，比特模式无法泄漏。图源：Yen et al. (2025)，Fig. 1。*

![[assets/figures/purcell-filter/16a63dc01ce863ef6a40103d2d3e0f74725d9f4fe51b66193e8dd901be894de7.jpg]]

*实测抑制性能：谐振器介导衰减的抑制因子随频率——2 个量级/400 MHz 带宽，凹陷点 Purcell 寿命 >16 ms。图源：Yen et al. (2025)，Fig. 2。*

**参量多元件耦合架构**把滤波器纳入更大的设计空间：单个耦合网络（谐振腔+参量泵浦）同时承担**相干控制**（门操作、态传输）与**耗散控制**（Purcell 抑制、读出通断）——参量泵浦的频率选择让同一网络在不同时刻呈现不同的等效电路，滤波/读出/操控不再需要分离的专用元件。

![[assets/figures/purcell-filter/ce76b38ad2e4933806f487e08506b13f8f3bf235353198128218b9e815265291.jpg]]

*参量多元件耦合架构：同一网络经参量泵浦切换等效电路——相干与耗散控制共用硬件。图源：Huber et al. (2024)，Fig. 1。*

![[assets/figures/purcell-filter/007733ef267a00170bb6cbee51889be0304a06adf99a383051c6627babd2dd14.jpg]]

*架构的性能验证：参量切换下的读出/滤波/门操作——多功能集成的定量指标。图源：Huber et al. (2024)，Fig. 2。*
